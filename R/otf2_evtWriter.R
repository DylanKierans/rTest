...
#' rTrace_init_ - doxygen documentation
#'  Simple wrapper for C++ otf2 init function
#' @param flag_debug Boolean - True to enable verbose stdout
#' @export
rTrace_init_EvtWriter <- function(flag_debug=F) {
    if (flag_debug) cat("Entering wrapper\n")
    init_EvtWriter()
    if (flag_debug) cat("Finished function.\n")
}

#' rTrace_finalize_Archive- doxygen documentation
#'  Simple wrapper for C++ otf2 init function
#' @param flag_debug Boolean - True to enable verbose stdout
#' @export
rTrace_finalize_EvtWriter <- function(flag_debug=F) {
    if (flag_debug) cat("Entering wrapper\n")
    finalize_EvtWriter()
    if (flag_debug) cat("Finished function.\n")
}
...
#' rTrace_evtWriter_Write - doxygen documentation
#'  More info
#' @param regionRef Int - regionRef
#' @param event_type Bool - True if enter, False if exit
#' @param flag_debug Boolean - True to enable verbose stdout
#' @export
rTrace_evtWriter_Write <- function(regionRef, event_type, flag_debug=F) {
    if (flag_debug) cat("Entering wrapper:\n")
